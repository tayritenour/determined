...
class DeepSpeedTrialController(det.TrialController):
    ...
    def _report_to_searcher(self, result):
        # only called once per trial, only in dsat paths
        logging.info("Reporting Completed...")
        ops = self.context._core.searcher.operations()
        op = next(ops)
        op.report_completed(result)
        # Emptying the operations() iterator is significant, even though you can't tell from here.
        try:
            next(ops)
        except StopIteration:
            logging.info("Successfully drained CustomSearcher Operations!")
            pass
        else:
            raise AssertionError("didn't expect extra operations!")

    def _do_the_thing(self):
        try:
            logging.info("HERE Doing the thing!")
            # Running training so that the trial with create the
            # model_info.json.
            _ = self._train_for_step(1, 1, 0)
            ...
            if self.is_chief:
                ...
                with path.open("r") as f:
                    ...
                    logging.info("ACTUALLY SENDING THE DICTIONARY!!!")
                    logging.info(f"{model_info_dict}")
                    self._report_to_searcher(model_info_dict)
                    # self.context.model_info = det_ds.ModelInfo(
                    # model_info = det_ds.ModelInfo(
    ...
    def _compute_validation_metrics(self) -> workload.Response:
        ...
        #             },
        #             # TODO: actually bring all the values
        #         }
        #     return {}
